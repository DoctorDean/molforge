...
- **Unimplemented format stubs** — `read`/`write` for PQR raise
  `NotImplementedError` with a clear pointer. The *import path* is
  committed; the implementation is planned. (SDF, MOL2, and PDBQT
  are now implemented.)
